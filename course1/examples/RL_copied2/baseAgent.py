...
class BaseAgent(metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        super(abc.ABCMeta, self).__new__(cls="wrestling-agent")

    @abc.abstractmethod
    def initFunc(self) -> None:
        r"""初始化函数的抽象，在__init__中被调用，方便重初始化

        Raises:
            NotImplementedError: 当子类没有initFunc报错
        """
    ...
    def act(self, obs: dict) -> List[float]:
        ...
        Raises:
            NotImplementedError: 当子类没有act报错
    ...
    @abc.abstractmethod
    def algo(self) -> List[float]:
        """ 算法核心部分，根据观察值与内部信息决定策略，返回策略决定的动作

        Raises:
            NotImplementedError: 当子类没有algo报错，可以根据不同的情况
...
'''
olympics_engine/tools/func.py的函数
作为参考
'''
...
def rotate(x, y, theta):
    ...
    """
    x_n = math.cos(theta * math.pi / 180) * x + \
        math.sin(theta * math.pi / 180) * y
    y_n = -math.sin(theta * math.pi / 180) * x + \
...
def line_intersect(line1, line2, return_p=False):
